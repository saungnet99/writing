    ...
    RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        // Remove app icon with purpose != any
        if (isset($pwa->icons)) {
            $pwa->icons = array_filter($pwa->icons, fn($icon) => $icon->purpose == 'any');
        }

        if (
            property_exists($payload, 'maskable_icon')
            && $payload->maskable_icon
            && isset($pwa->icons[0])
        ) {
            $icon = clone $pwa->icons[0];
            $icon->purpose = 'maskable';
            array_unshift($pwa->icons, $icon);
        }
