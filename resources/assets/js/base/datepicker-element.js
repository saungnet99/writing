...
export class DatePickerElement extends HTMLElement {
    ...
    render() {
        ...
        <div class="relative" x-data='datepicker(${data})' @click.outside="showDatepicker = false" @focusout="handleFocusOut">
                <div class="relative">
                    <input type="text" id="${this.id || ''}" class="pr-11 cursor-pointer input" readonly x-ref="input" placeholder="${placeholder || ''}" @focus="showDatepicker = true" @click="showDatepicker = true" x-model="datepickerValue"/>
                    <template x-if="options.value">
                        <button type="button" @click="clearValue()" class="block absolute right-3 top-1/2 text-xl -translate-y-1/2 ti ti-x text-content-dimmed hover:text-content"></button>
                    </template>

                    <template x-if="!options.value">
                        <i class="block absolute right-3 top-1/2 text-2xl -translate-y-1/2 pointer-events-none ti ti-calendar text-content-dimmed"></i>
                    </template>
                        ...
                        <div class="flex flex-col gap-3">
                            <div class="flex justify-between items-center">
                                ...
                                <div class="flex items-center">
                                    <button type="button" class="transition duration-100 ease-in-out cursor-pointer ti ti-chevron-left text-content-dimmed hover:text-content" :disabled="isFirstMonth()" @click="previousMonth()"></button>

                                    <button type="button" class="transition duration-100 ease-in-out cursor-pointer ti ti-chevron-right text-content-dimmed hover:text-content" :disabled="isLastMonth()" @click="nextMonth()"></button>
                                </div>
                            </div>
